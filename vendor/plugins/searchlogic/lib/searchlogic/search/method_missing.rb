  ...
  class Search
    module MethodMissing
      ...
      private
        ...
        def cast_type(name)
          scope_options = scope_options(name)
          arity = klass.scope_arity(name)
          if !arity || arity == 0
            :boolean
          else
            scope_options.respond_to?(:searchlogic_options) ? scope_options.searchlogic_options[:type] : :string
          end
        end
